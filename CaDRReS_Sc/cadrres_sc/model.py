...
def create_placeholders(n_x_features, n_y_features, sample_weight=False):
    ...
    """
    # Gene expression
    X = tf.keras.Input(shape=(n_x_features,), dtype=tf.float32)
    # Drug response
    Y = tf.keras.Input(shape=(n_y_features,), dtype=tf.float32)
    
    if sample_weight:
        # For logistic weight based on maximum drug dosage
        O = tf.keras.Input(shape=(None,), dtype=tf.float32)
        # For indication-specific weight
        D = tf.keras.Input(shape=(None,), dtype=tf.float32)
        return X, Y, O, D
    else:
        return X, Y
